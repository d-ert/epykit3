...
| Lint and types | `uv run --frozen ruff check src/ tests/` and `uv run --frozen mypy src/epykit` | Rules F, I, W, UP, E, B and RUF plus a C901 complexity ceiling of 32; see `[tool.ruff.lint]` in `pyproject.toml`. |
