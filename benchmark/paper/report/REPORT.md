...
> **Confidence intervals.** The point estimates in §2.1–§2.3 are reproduced
> with their PROTOCOL R2 95 % CIs — Wilson on TPR/FPR, bootstrap (B = 1,000)
> on F1/AUROC, Wilson on DMR recall/precision — in [paper.md](../paper/paper.md)
> Tables 1–3. Both the points and the intervals are regenerated by
> [`scripts/promote_cis.py`](../scripts/promote_cis.py) →
> [`data/headline_cis.json`](../data/headline_cis.json) from the committed
> per-CpG parquets; the bare points are retained here for compact auditing.
