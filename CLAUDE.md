...
Four callers — DSS-compatible `chain_merge` (default) with presets (`strict`/`default`/`permissive`), tile-based (read-pooled), sliding-window with signed Stouffer combining, and HMM segmentation. Permutation empirical FDR is implemented (API `tl.dmr`) for `method='tile'` and `method='chain_merge'`; each shuffles labels, re-runs the caller, and adds `empirical_pvalue` / `empirical_qvalue` / `empirical_fdr_set`. **`empirical_qvalue` defaults to a count-ratio target-decoy FDR** (`fdr_method='region'`, BSmooth/SAM: `mean(#null survivors ≤ t)/(#observed ≤ t)`, dispersion cancels in the ratio); `fdr_method='max_t'` selects the legacy Westfall-Young min-P/FWER (which collapses to ~1.0 under realistic dispersion). Shared core: `_region_count_ratio_fdr` (stats) + `_aggregate_region_perm_results` (region/max_t dispatch) in `dmr.py`, both caller-agnostic. `chain_merge` recomputes the full per-CpG DMC per shuffle (`empirical_fdr_for_chain_merge`, simple two-group tests only) — **expensive (hours at genome scale)**. `sliding_window` / `segment` still raise `NotImplementedError`; the CLI is tile-only. See `docs/review/2026-06-08-region-empirical-fdr-design.md`.
