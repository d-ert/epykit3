...
**`lr+` power stack** is four opt-in enhancements on top of `lr` that are README- and benchmark-validated to close the gap to methylKit / RADMeth / DSS: `fdr_method="fdr_tsbh"`, `neighbour_combine=True`, `sep_fallback=True`, `dispersion="eb"`. When `neighbour_combine=True`, **`pvalue`/`qvalue` become the combined values; raw per-CpG p-values move to `pvalue_raw`/`qvalue_raw`**. Downstream code that consumes `pvalue` expecting the per-CpG raw must switch to `pvalue_raw`. These options are currently Python-API-only — the CLI flags are pending.
